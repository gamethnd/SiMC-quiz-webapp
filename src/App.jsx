...
export default function App() {
  ...
  const [deleteSubjectId, setDeleteSubjectId] = useState(null); // Subject pending delete confirmation
  ...
  useEffect(() => {
    ...
    if (timeLeft === activeWarningTime) {
      ...
      setNotification({ type: 'warning', message: `${formatWarningTime(activeWarningTime)} remaining`, duration: 5000 });
    }
  ...
  const startQuiz = async () => {
    ...
    // Mode: Own (Default)
      ...
      // Two modes, decided automatically by pool size:
      // - pool larger than the requested count -> random sample from the bank
      // - pool equal to (or smaller than) the count -> fixed set, shown in import order
      if (uniquePool.length > limit) {
        finalQuestions = shuffleArray(uniquePool);
      } else {
        finalQuestions = [...uniquePool].sort((a, b) => {
    ...
    // 3. Final Limit
    // Composite mode mixes sources, so it always shuffles here.
    // Own mode already decided order/shuffle above by pool size: a fixed set
    // (pool <= limit) must keep its import order, so don't re-shuffle it.
    const isFixedOwnSet =
      subjectData?.sourceMode !== 'composite' && finalQuestions.length <= limit;
    const ordered = isFixedOwnSet ? finalQuestions : shuffleArray(finalQuestions);
    setActiveQuizQuestions(ordered.slice(0, limit));
    ...
    try {
      ...
      const promises = parsed.map((q, i) => {
        ...
        return addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'quiz_pool'), {
          ...
          order: i, // Preserve import order so a fixed-set quiz can show questions in sequence
          subjectId: activeSubject,
          createdAt: serverTimestamp()
        });
      });
      ...
      // Re-fetch the pool so the newly imported questions show up immediately.
      // quiz_pool is read once via getDocs (not a live snapshot), so without
      // this the list below stays stale until the teacher reloads the view.
      const refreshed = await getDocs(query(
        collection(db, 'artifacts', appId, 'public', 'data', 'quiz_pool'),
        where('subjectId', '==', activeSubject)
      ));
      setQuestionsPool(refreshed.docs.map(d => ({ id: d.id, ...d.data() })));
    ...
    if (notification.type === 'warning') {
      return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 pointer-events-none">
          <div className="bg-amber-50 border border-amber-200 rounded-2xl shadow-2xl px-6 py-5 sm:px-7 sm:py-6 flex items-center gap-4 text-center max-w-[88%] sm:max-w-xs w-full">
            <div className="bg-amber-100 rounded-full p-3 flex-shrink-0">
              <AlertTriangle className="w-8 h-8 sm:w-9 sm:h-9 text-amber-600 animate-pulse" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="font-bold text-amber-800 text-lg sm:text-xl leading-tight">Time Running Out</p>
              <p className="text-amber-700 font-semibold text-base sm:text-lg leading-tight mt-0.5 whitespace-nowrap">{String(notification.message)}</p>
            </div>
          </div>
        </div>
      );
  ...
  const DeleteSubjectModal = () => {
    if (!deleteSubjectId) return null;
    const sub = subjects.find(s => s.id === deleteSubjectId);
    return (
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-2xl max-w-[90%] sm:max-w-md w-full p-5 sm:p-6">
          <h3 className="text-lg sm:text-xl font-bold text-slate-800 mb-2">Delete Subject?</h3>
          <div className="mb-4 sm:mb-6 bg-amber-50 border border-amber-200 rounded-lg p-3 sm:p-4 flex gap-2 sm:gap-3 items-start">
            <AlertTriangle className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600 flex-shrink-0 mt-0.5" />
            <div>
              <p className="font-bold text-amber-800 text-sm sm:text-base">Delete{sub ? ` "${sub.name}"` : ' this subject'}?</p>
              <p className="text-xs sm:text-sm text-amber-700 mt-0.5 sm:mt-0">Questions will remain but be hidden.</p>
            </div>
          </div>
          <div className="flex gap-2 sm:gap-3">
            <button onClick={() => setDeleteSubjectId(null)} className="flex-1 py-2 sm:py-3 px-3 sm:px-4 rounded-lg sm:rounded-xl border border-slate-200 font-bold hover:bg-slate-50 text-sm sm:text-base">Cancel</button>
            <button onClick={confirmDeleteSubject} className="flex-1 py-2 sm:py-3 px-3 sm:px-4 rounded-lg sm:rounded-xl bg-red-600 text-white font-bold shadow-lg hover:bg-red-700 text-sm sm:text-base">Delete Subject</button>
          </div>
        </div>
      </div>
    );
  ...
  const filteredQuestions = React.useMemo(() => {
    return questionsPool
      .filter(q => q.subjectId === activeSubject && q.text.toLowerCase().includes(questionSearchTerm.toLowerCase()))
      // Show questions in import order. Questions imported before `order`
      // existed (or added one-by-one) have no order field — fall back to
      // createdAt so they still sort stably instead of by random doc id.
      .sort((a, b) => {
        const ao = a.order, bo = b.order;
        if (ao !== undefined && bo !== undefined) return ao - bo;
        if (ao !== undefined) return -1;
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg w-96">
            ...
            <div className="mb-6">
              <input
                ...
                placeholder="Student ID / SAP ID"
                inputMode="numeric"
                value={studentId}
                onChange={e => { setStudentId(e.target.value.replace(/\D/g, '')); setStudentLoginError(false); }}
              />
